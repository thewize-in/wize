...
export interface IPatientEntryBookRepo extends Repo<PatientEntryBook> {
  getPatientEntryBookByBookId(
    bookId: string
  ): Promise<Result<PatientEntryBook>>;
  getPatientEntryBook(bookId: string): Promise<Result<any>>;
...
export class PatientEntryBookRepo implements IPatientEntryBookRepo {
  ...
  async save(patientEntryBook: PatientEntryBook): Promise<void> {
    ...
    const rawPatientEntryBook = PatientEntryBookMap.toPersistence(
      patientEntryBook
    );
  ...
  async exists(bookId: string): Promise<boolean> {
    try {
      const patientEntryBookExist = await this.model.findOne({
        book_id: bookId,
      });
      const guardResult = Guard.againstNullOrUndefined(
        patientEntryBookExist,
        'patientEntryBookExist'
      );
  ...
  async getPatientEntryBookByBookId(
    bookId: string
  ): Promise<Result<PatientEntryBook>> {
    ...
    if (!guardResult.succeeded)
      return Result.fail<PatientEntryBook>(
        '[getPatientEntryBookByBookId]: not found'
      );

    return Result.ok<PatientEntryBook>(
      PatientEntryBookMap.toDomain(rawPatientEntryBook)
    );
  }
  async getPatientEntryBook(bookId: string): Promise<Result<any>> {
    const rawPatientEntryBook = await this.model.findOne({ book_id: bookId });
    const guardResult = Guard.againstNullOrUndefined(
      rawPatientEntryBook,
      'rawPatientEntryBook'
    ...
    return Result.ok<any>(PatientEntryBookMap.toDTO(rawPatientEntryBook));
  }
  async getAllPatients(bookId: string): Promise<Result<any>> {
    const patientListResult: any = await this.model.findOne(
      { book_id: bookId },
      { patient_list: 1 }
    );
    ...
    if (!guardResult.succeeded)
      return Result.fail<any>('patientList not found');

    return Result.ok<any>({ allPatients: patientListResult.patient_list });
  }
  async getDonePatients(bookId: string): Promise<Result<any>> {
    const donePatientListResult: any = await this.model.findOne(
      { book_id: bookId },
      { done_patient_list: 1 }
    );

    const guardResult = Guard.againstNullOrUndefined(
      donePatientListResult,
      'donePatientListResult'
    );
    if (!guardResult.succeeded)
      return Result.fail<any>('donePatientList not found');

    return Result.ok<any>({
      donePatients: donePatientListResult.done_patient_list,
    });
  }
  async getUndonePatients(bookId: string): Promise<Result<any>> {
    const undonePatientListResult: any = await this.model.findOne(
      { book_id: bookId },
      { undone_patient_list: 1 }
    );

    const guardResult = Guard.againstNullOrUndefined(
      undonePatientListResult,
      'undonePatientListResult'
    );
    if (!guardResult.succeeded)
      return Result.fail<any>('undonePatientList not found');

    return Result.ok<any>({
      undonePatients: undonePatientListResult.undone_patient_list,
    });
  }
}
